...
const getRollResultHeader = (
  ...
  if (rollFor === c.SAVINGTHROW) {
    ...
    resultHeader =
      rollFor.toUpperCase() +
      " " +
      game.i18n.localize("wh4e.dice.taskRollVsTarget");
  }
  return (
    resultHeader +
    " " +
...
const getResultCategory = (
  ...
  if (diceOutcome === c.SUCCESS) {
    let extremeRollQuality =
      game.i18n.localize("wh4e.dice.withQuality") + " " + rollResult.toString();
    if (
      diceOne === diceTwo &&
    ...
    if (rollTarget >= 20) {
      extremeRollQuality =
        game.i18n.localize("wh4e.dice.withQuality") +
        " " +
    ...
    if (rollFor) {
      // Only push on quality for SV and Task Checks
